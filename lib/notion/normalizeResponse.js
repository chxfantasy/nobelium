...
function normalizeEntries (entries) {
  ...
  for (const [key, entry] of Object.entries(entries)) {
    if (entry?.value?.value?.id) {
      ...
      normalized[key] = { value: entry.value.value, role: entry.role ?? 'reader' }
    } else if (entry && typeof entry === 'object' && 'role' in entry && entry.role === undefined) {
      normalized[key] = { ...entry, role: 'reader' }
    } else {
